...
class KinematicChain:
    ...
    def __init__(self, structure, base_lnk, tip_lnk):
        ...
        self.active_mask = self._compiled.active_jnt_ids_mask[self.jnt_ids]
        self.active_jnt_ids = self.jnt_ids[self.active_mask]
        # active joint limits
    ...
    def _check_mmc(self):
        mmc_flags = self._compiled.mmc_src_by_idx[self.jnt_ids] >= 0
        if np.any(mmc_flags):
            bad_ids = self.jnt_ids[mmc_flags]
