...
- **手剥离（已做）**：原 h0602.urdf 把两只 Linkerbot O6 灵巧手（各 11 指关节）硬编码进 body。已切成独立 EE `end_effectors/linkerbot/o6`（`o6_left.urdf`/`o6_right.urdf` + `O6Left`/`O6Right` 两个 MechBase），body URDF 砍到 `*_arm_link_6` 收尾。L1 `__init__` 用 `mount(O6Left(), lnk('left_arm_link_6'), z=0.034)` 把手挂回去——位姿与原一体 URDF **逐点一致（diff=0）**。每只手带两个 center tcp：`power_grasp_center`（全手握爪中心）/ `pinch_center`（拇指-食指捏取中心），抓取走跨对象 ik `robot.ik('left_arm', robot.left_hand.tcp('power_grasp_center'), R, p)`。指链 ik 暂未建模（手当刚体 EE 用）。一次性迁移脚本在 `tools/extract_o6_hands.py` / `tools/strip_o6_hands.py`。
...
| ~~部分 IK（位置/轴约束）~~ | **已完成**（§7.5a）：单独的 `MechBase.ik_partial(chain, tcp, tgt_pos=, axis_constraints=, …)`,数值解,返回 qs 列表。**没有**揉进 `ik`——避免 `tgt_rotmat` 与 `axis_constraints` 共存的混乱(分开写更直观)。 | ✅ |
...
- [x] **4.** 其余 7 个 arm（ur3/rs007l/lite6/fr3/crx5ia/openarm OALft+OARgt）全部转继承 `MechBase` + `add_chain('main', …, solver=…)` + `add_tcp('flange')`，删各自 `_init_solver` override。analytic 臂传 solver 类(P234X56/S456X12/CRX5iaAnalyticIK),FR3 默认 SELIK + flange 偏移 0.107 进 tcp,openarm 共享 `data_la` 库。顺手修了 openarm 的 `kinematics`→`kine` import 笔误。验证:8 个臂类全部 load + solver 类型 + ik round-trip 正确。
  - **4b.** `_init_solver` 折叠进 `get_solver`(惰性建默认 SELIK);`ik`/`ik_partial` 改调 `get_solver`。自定义 solver 走 `add_chain(solver=)` 配对,不再有 override 钩子。
  - **4c.** 共享 `motion/trajectory/cartesian.py`:`ik_tcp_nearest` → `ik('main','flange',…,max_solutions=1)`,chain/tcp 名做成参数。
...
- [x] **6.** 删除 `manipulator_base.py` + `attached_frame.py`（无人 import,`git rm`）。robot `__main__`/工厂 + examples 的旧 API 调用由子代理迁移中（ik_tcp→ik / engage→mount / gl_tcp_tf→tcp('flange').tf）。coripps 已是新 API。
...
- [x] **E3.** 5 个末端转 `MechBase + Mixin`：FR3Gripper/OR2FG7/OAGripper/Rtq2F85（GripperMixin，注册 `'grasp_center'`）、ORSD（PointMixin，注册 `'tcp'`）。`PointMixin` 加 `_tcp_loc_tf()`。各自 `__main__` 的 `gl_tcp_tf`→`tcp(...).tf`。验证：load + grip_at/touch_at 走注册表 + clone。
- [x] **E4.** `grasp/antipodal.py` 的 `loc_tcp_tf` → `tcp('grasp_center').loc_tf`（一处）；`polypodal.py` 只用 `contact_pattern`（已保留）。
- [x] **E5.** 删 `EndEffectorBase`（`ee_base.py` 仅留 `GripperMixin`/`PointMixin`），去掉两个 helper 的 `loc_tcp_tf` 兼容回退。无 `EndEffectorBase`/`loc_tcp_tf` 残留。
- [x] **E6.** 夹爪-IK examples retarget `'flange'`→`gripper.tcp('grasp_center')`（恢复旧 `engage(auto_tcp)` 语义）：9 个 example 改瞄抓取点,裸臂保留 flange。66 个 example 语法全过。

### 下游项目 + 清理
- [x] **D1.** **coripps** 迁移（见 §7.5c)。
- [x] **D2.** **kurabo** 迁移（9 文件）：`krb_left`/`krb_right` 夹爪 → `MechBase+GripperMixin`+`grasp_center`；`robot.py`/`robot_fanuc.py`（`LeftRobot/RightRobot(UR3/CRX5ia)`）`_main_chain`→`chain('main')`、`engage`→`mount`、`gl_tcp_tf`→`gripper.tcp('grasp_center').tf`；`grasp.py` `loc_tcp_tf`→`tcp('grasp_center').loc_tf`；3 个 assembly 脚本 + planning_test `ik_tcp`→`ik('main', arm.gripper.tcp('grasp_center'), …)`。验证：import + 构造 + 跨对象 ik 全过。
- [x] **D3.** 删 `crx5ia.py` 的死代码 `CRX5iaNumIKSolver`（非主链兜底,现走默认 SELIK,无人引用）+ `orbkn` import。

> **完成。** `ManipulatorBase` / `EndEffectorBase` / `attached_frame.py` 三者全删。`one`(7 臂 + 5 末端 + cartesian/antipodal + ~40 examples) + coripps + kurabo 全部迁到统一 `MechBase` + chain/tcp/ik 模型。所有机构调用一律 `mech.ik(…)` / `ik_partial(…)`；solver 与 chain 在 `add_chain(solver=)` 成对声明；默认 SELIK 由 `get_solver` 惰性建。

### D4. ik/工具函数 **pos-先 + 默认 chain/tcp**（对齐 ROS Pose 习惯）
- `ik(tgt_pos, tgt_rotmat, chain='main', tcp='flange', …)` —— **pos 在前**(ROS `Pose`={position, orientation} 习惯),chain/tcp 带默认 → 单臂 `arm.ik(pos, rotmat)`;多链/跨对象 `arm.ik(pos, rotmat, chain='left_arm', tcp=gripper.tcp('grasp_center'))`。
- `ik_partial(tgt_pos, axis_constraints, chain='main', tcp='flange', …)` —— 同样 pos 先。
- 工具函数改名 + 换序(**改名让漏改处大声报错**):`tf_from_rotmat_pos(rotmat,pos)` → **`tf_from_pos_rotmat(pos, rotmat)`**(带形状守卫,传反 → ValueError);`set_rotmat_pos(rotmat,pos)` → **`set_pos_rotmat(pos, rotmat)`**。
- 范围 ~165 处(one/examples/coripps/kurabo),子代理分目录迁移 + grep 旧名残留=0 + swap 守卫兜底静默错。
