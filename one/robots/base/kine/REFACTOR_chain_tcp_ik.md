...
- [ ] **5.** 偏 IK：`ik_attached_frame` → `MechBase.ik(chain, tcp, …, axis_constraints=…)`；`AttachedFrame` 调用点迁到 `TCP`。可视化助手 `toggle_tcp` 等搬到 `MechBase`。
- [ ] **6.** 全部 arm 迁完后，**删除 `manipulator_base.py` 整个文件** 与 `attached_frame.py`。
- [ ] **7.** `L1`：用 `add_chain('left_arm', …)` 等 + `add_tcp(...)`，验证三类（arm / 人形 / 手）`ik('chain_name', 'tcp_name', pose)` 同一套。**注意命名冲突**：`L1` 现有 `chain(base_name, tip_name)` 两参构造方法会遮蔽 `MechBase.chain(name)`——迁移时把它改成 `add_chain` 调用（或重命名为 `make_chain`），否则 `L1` 上 `ik('left_arm', …)` 会报参数错。
