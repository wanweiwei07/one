...
def gen_cylinder(spos=np.zeros(3), epos=np.ones(3) * 0.01,
    ...
    if key in _primitive_cache:
        ...
        geometry = geom.Geometry(verts=verts, faces=faces)
    ...
    return_obj.set_rotmat_pos(rotmat, spos)
    return return_obj
...
def gen_frame(pos=np.zeros(3), rotmat=np.eye(3),
    ...
    return_obj.add_visual(mdl.Model(geometry=geometry, rotmat=rotmat_x, rgb=color_mat[:, 0], alpha=alpha))
