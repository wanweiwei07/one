...
def revolve(profile, segments=36):
    ...
    if has_bottom:
        center_bottom = np.array([profile[0, 0], profile[0, 0], profile[0, 1]], dtype=np.float32)
    else:
    ...
    if has_top:
        center_top = np.array([profile[-1, 0], profile[-1, 0], profile[-1, 1]], dtype=np.float32)
    else:
