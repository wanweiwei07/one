...
def prepare_ms():
    ...
    jnt_lf = orbms.Joint(
        ...
        pos=np.array([0, -0.0113, 0], dtype=np.float32),
        lmt_lo=0.0, lmt_up=0.0113)
    jnt_rf = orbms.Joint(
        jnt_type=ouc.JntType.PRISMATIC,
...
class KRBRight(oreb.EndEffectorBase, oreb.GripperMixin):
    ...
    def __init__(self):
        super().__init__(
            loc_tcp_tf=oum.tf_from_rotmat_pos(pos=(0.004, 0, 0.235)))
        self.jaw_range = np.array([0.0, 0.0226], dtype=np.float32)  # min, max
        self.open_dir = ouc.StandardAxis.Y
        self.contact_pattern = np.array(
            [[-0.00485, 0.0005, 0.0],
             [0.00485, 0.0005, 0.0]], dtype=np.float32)
...
if __name__ == '__main__':
    ...
    gripper = KRBRight()
    gripper.set_jaw_width(0.0226)
    gripper.attach_to(base.scene)
    base.run()

    # jaw_widths = [0.0, 0.006, 0.012, 0.018, 0.0226]
    # template = KRBRight()
    # grippers = []
    # phases = []
    ...
    # base.schedule_interval(update, interval=1.0 / 1.0, t=t, grippers=grippers, phases=phases)
    # base.run()
